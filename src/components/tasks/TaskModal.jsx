...
export default function TaskModal({ open, onClose, task = null }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        streak:       isEdit ? task.streak : 0,
        completedDates: isEdit ? task.completedDates : [],
      }
      if (isEdit) {
  ...
  const sel = (condition) => ({
    border: `1.5px solid ${condition ? '#14b8a6' : '#e2e8f0'}`,
    background: condition ? '#f0fdfa' : '#f8fafc',
    color: condition ? '#0f172a' : '#64748b',
  })
          ...
          >
            ...
            <div style={{ padding: '20px 24px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                ...
                <input
                  style={{ ...input, borderColor: errors.title ? '#ef4444' : '#e2e8f0' }}
              ...
              <div>
                ...
                <textarea
                  style={{ ...input, resize: 'none', minHeight: 68 }}
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {FOCUS_AREAS.map(area => (
                    <button
                      key={area.id}
                      type="button"
                      onClick={() => set('focusArea', area.id)}
                      style={{
                        padding: '5px 12px', borderRadius: 99, fontSize: 12, fontWeight: 500,
                        cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s', border: '1.5px solid',
                        borderColor: form.focusArea === area.id ? area.color : '#e2e8f0',
                        background: form.focusArea === area.id ? `${area.color}15` : '#f8fafc',
                        color: form.focusArea === area.id ? area.color : '#64748b',
                      }}
                    >
                      {area.label}
                    </button>
                    ...
                    {PRIORITIES.map(p => (
                      <button
                        ...
                        style={{
                          ...
                          borderColor: form.priority === p ? PRIORITY_COLORS[p] : '#e2e8f0',
                          background: form.priority === p ? `${PRIORITY_COLORS[p]}12` : '#f8fafc',
                  ...
                  <div style={{ display: 'flex', gap: 4, marginBottom: 10 }}>
                    {WEEKDAYS.map(day => (
                      <button
                        ...
                        style={{
                          flex: 1, padding: '6px 2px', borderRadius: 7, fontSize: 11,
                          fontWeight: 500, border: '1.5px solid', cursor: 'pointer',
                          fontFamily: 'inherit',
                          ...sel(form.scheduleDays.includes(day)),
                        }}
                      >
                        {day}
                      </button>
              ...
              <div style={{ display: 'flex', gap: 10, paddingTop: 4 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    border: 'none', color: '#fff', fontSize: 13.5, fontWeight: 500,
